...
ground truth that also resolve nowhere) by $3$--$11\times$ over CARD at a
...
sometimes increases them\ugr{,} so the structural signal addresses a failure mode
...
\sugg{\noindent\textbf{Answer (RQ1).} Cross-file retrieval helps substantially
\emph{on average} on every surface and model, but not uniformly per instance: a
non-trivial share of completions is left unchanged or actively degraded, so an
always-retrieve policy wastes part of its budget --- selective retrieval is
justified.}
...
curves rises steeply and then flattens, so a confidence gate buys most of
...
longer code and thus emit more unresolvable names. A policy that only optimizes confidence/ES
therefore leaves a structural failure mode untouched. This is the gap our static
gate targets.

\sugg{\noindent\textbf{Answer (RQ2).} A small retrieval budget buys most of
always-retrieve's accuracy --- the confidence gate trades cost for accuracy
well --- but the gate is blind to structural validity: invented identifiers
persist, and can even increase, under retrieval, leaving exactly the failure
mode a structural signal can address.}
...
is what it produces.} Adding the static-analysis gate retrieves an extra $2$--$9\%$ of instances --- precisely those whose confident zero-shot output references an unresolvable name --- and cuts the invented-identifier rate by \textbf{$3.6\times$ to $11.3\times$} (to zero on RepoEval-1.5B). The effect is consistent across all
...
approach always-retrieve (where there is nothing left to select). Figure~\ref{fig:cascade} summarises the headline reduction as paired bars. Because the trigger and the
...
\sugg{\noindent\textbf{Answer (RQ4).} The gate is effectively free: by
construction and empirically it never lowers edit similarity at any threshold,
adds only single-digit-percent retrieval, and its benefit concentrates exactly
