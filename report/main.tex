...
identifier used in $\hat{y}_0$ that is not defined anywhere in the current file
(Section~\ref{sec:analyzer}). If the check fires}, we retrieve and regenerate
exactly as in Stage 2. Otherwise we commit $\hat{y}_0$ as the final prediction.
...
retrieval fires; BM25 is not re-run at inference. The static-analysis gate does
not use these chunks --- it resolves identifiers against the in-file context
$X$ alone.}
...
appear; see Section~\ref{sec:dataset}.)} Rather than shipping a pre-retrieved chunk list per instance, as CrossCodeEval does, RepoEval ships the upstream repositories on disk in full. Thus, BM25 is run at inference time with 20-line sliding windows at stride 10 computed live from the source files, with the top ten matches returned per query. \new{The repository's Python files are loaded only to drive this live BM25 retrieval; as on CrossCodeEval, the static-analysis gate consults only the in-file context $X$, not a repository-wide symbol table.} We adopt these settings unchanged so our reimplementation of the CARD baseline remains directly comparable to its published numbers~\cite{zhang2024card}.
...
A confident call to a helper that exists nowhere is exactly the structural
...
rather than a hand-written analyzer, and the procedure is deliberately simple:}
...
every name against the file's own bindings --- imports, function/class
...
any caused by the surrounding context) and \emph{fire} the gate --- triggering
retrieval --- if at least one remains.}

\new{The check is purely in-file and resolution-only: it consults the imports and
definitions already present in $X$, needs \emph{no} repository-wide symbol table
and \emph{no} code execution, and runs in milliseconds. Two properties matter for
...
a malformed completion can never manufacture a spurious trigger. This pyflakes
gate --- not a multi-check analyzer --- is what produces every cascade ($C4$)
