...
retrieval whenever the prediction falls under one of three classes of
hallucination that the logit signal missed.
...
parse $\hat{y}_0$ with tree-sitter and run three independent checks
against the repository's symbol table. If any
check fires, we retrieve and regenerate exactly as in Stage 2. Otherwise
...
\subsection{Tier 1: Out-of-scope identifiers}
\label{sec:tier1}
\rev{Tier~1 targets the most common structural hallucination: a call or reference
to a name that is not defined anywhere visible at the hole. We parse $\hat{y}_0$
and collect identifiers appearing in \emph{significant positions} --- a call
target, an attribute receiver, a subscript value, a class base, a bare decorator,
an exception type, or a \texttt{raise} target --- positions where the identifier
must denote a real, bound object for the code to run. An identifier in such a
position fires Tier~1 if it is not resolvable in the lexical scope of the current
file (it is not a builtin, not imported, and not bound by a preceding
definition, assignment, parameter, comprehension, \texttt{with}/\texttt{except}
alias, etc., in $X_{\text{left}}$/$X_{\text{right}}$). Tier~1 is purely a scope
check and needs no repository symbol table. In our evaluation we realise this
check with pyflakes' undefined-name analysis (F821) over the reconstructed file,
which gives a binding-complete, false-positive-resistant version of the same
signal; this is the gate isolated in all reported cascade results.}

\subsection{Tier 2: Signature mismatches}
\label{sec:tier2}
\rev{Tier~2 catches calls to functions that \emph{do} exist in the repository but
are invoked incorrectly. For every call in $\hat{y}_0$ whose callee can be
resolved in the repository symbol table, we compare the call site against the
callee's declared signature (arity and keyword names) and fire on an
incompatibility (e.g., too many positional arguments, or an unknown keyword).
Unlike Tier~1, Tier~2 consults the symbol table built from the cross-file
context.}

\subsection{Tier 3: Wrong-origin imports}
\label{sec:tier3}
\rev{Tier~3 validates the provenance of imported names and module attributes
against the repository's module map: it fires when a symbol is imported from a
module that does not export it, or an attribute is accessed on a module that does
not define it. Tiers~2 and~3 are implemented in our in-house tree-sitter analyzer
and broaden the gate's coverage; because Tier~1 is by far the dominant and most
robust signal, the experiments in this paper isolate the Tier~1 gate and leave a
per-tier ablation to future work (Section~\ref{sec:future}).}
...
the cascade (C3, plus retrieve when the Tier-1 static gate fires on a skipped
...
would see. The same principle governs the cascade's Tier-1 trigger.
...
\textbf{Adaptive gate.} CARD's estimator is a LightGBM
regressor~\cite{ke2017lightgbm} over a 13-dimensional aggregation of per-token
probabilities and entropies that predicts the zero-shot output's ES, reproduced
from the original paper~\cite{zhang2024card}. It is calibrated once per model on
samples from The Stack (deduplicated)~\cite{kocetkov2022stack} and reused across
all datasets (the estimator is a property of the generator, not the benchmark). The
Tier-1 static gate is realised with pyflakes over the reconstructed file. Retrieval
...
nothing). Adding the Tier-1 static gate retrieves an extra $2$--$9\%$ of instances
...
For a few percent of extra retrieval (r\%), the Tier-1 static gate cuts the
...
budget on the structurally broken outputs}, which is exactly what the Tier-1 gate
...
\label{sec:future}
Several directions follow directly. First, a \textbf{per-tier ablation}: we isolated
the Tier-1 (out-of-scope) gate; Tiers~2--3 (signature and import-origin checks,
Sections~\ref{sec:tier2}--\ref{sec:tier3}) should be measured separately, as they
target rarer but higher-confidence structural errors. Second, \textbf{repair instead
...
Tier-1 to significant positions and by computing the signal on the reconstructed
